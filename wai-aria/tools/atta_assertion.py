...
class AttaAssertion:
    ...
    def __init__(self, obj, assertion, atta):
        self._atta = atta
        self._obj = obj
        self._as_string = "%s %s %s '%s'" % (*assertion[0:3], assertion[3])
        self._test_class = assertion[0]
        self._test_string = assertion[1]
        self._expectation = assertion[2]
        self._expected_value = self._atta.convert_platform_string(assertion[3])
    ...
    def __str__(self):
        ...
        return "\n\n{self._labels[0]:>{width}} {self._as_string}" \
               ...
               "\n{self._labels[3]:>{width}} {messages}\n".format(
                   width=label_width,
                   self=self,
                   actual_value="'%s'" % _wrap(self._actual_value),
    ...
    def _get_result(self):
        ...
        if self._expectation == self.EXPECTATION_IS:
            ...
            result = not (self._actual_value and self._expected_value in self._actual_value)
        ...
        if result:
            ...
            self._messages.insert(0, "Actual value: '%s'" % self._actual_value)
            self._bug = self._atta.get_bug(self._as_string, self._expected_value, self._actual_value)
            if self._bug:
                info = self._atta.get_bug_details(self._bug) or ""
                self._messages.append(": ".join((self._bug, info)))
...
class AttaPropertyAssertion(AttaAssertion):
    ...
    def __init__(self, obj, assertion, atta):
        super().__init__(obj, assertion, atta)
        if self._expected_value.startswith("[") and self._expected_value.endswith("]"):
            self._expected_value = self._atta.value_to_string(re.split("[, ]+", self._expected_value[1:-1]))
